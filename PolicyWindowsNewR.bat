...
:CUSTOMIZE_USERS
    setlocal
        ...
        rem Schedule Disable Admin middle
        ...
        rem Cấu hình đồng bộ thời gian cho hệ thống
        net stop w32time
        w32tm /unregister
        w32tm /register
        net start w32time
        ...
        rem Tự động khóa màn hình sau 5 phút không sử dụng máy tính
        ...
        rem Always on Realtime Defender
...
:VARIABLE_GPO
    ...
    set "REG_ParentLogs=HKEY_LOCAL_MACHINE\Software\Policies\Microsoft\Windows\EventLog"
...
:Working_GPO    
    ...
    rem Ghi log các sự kiện đăng nhập vào máy chủ
    auditpol /set /subcategory:Logon /success:enable /failure:enable

    rem Ghi log các sự kiện liên quan đến quản lý tài khoản và nhóm tài khoản
    ...
    rem Ghi log những thay đổi trong “Security State Change” và “System Integrity”
    auditpol /set /subcategory:"Security State Change" /success:enable /failure:enable
    auditpol /set /subcategory:"System Integrity" /success:enable /failure:enable
    
    rem Logs Application
    REG ADD %REG_ParentLogs%"\Application" /v MaxSize /t REG_DWORD /d 1024000 /f
    REG ADD %REG_ParentLogs%"\Application" /v File /t REG_SZ /d "%SystemDrive%\Logs" /f
    REG ADD %REG_ParentLogs%"\Application" /v AutoBackupLogFiles /t REG_SZ /d 1 /f
    rem Logs Security
    REG ADD %REG_ParentLogs%"\Security" /v MaxSize /t REG_DWORD /d 1024000 /f
    REG ADD %REG_ParentLogs%"\Security" /v File /t REG_SZ /d "%SystemDrive%\Logs" /f
    REG ADD %REG_ParentLogs%"\Security" /v AutoBackupLogFiles /t REG_SZ /d 1 /f
    rem Logs System
    REG ADD %REG_ParentLogs%"\System" /v MaxSize /t REG_DWORD /d 1024000 /f
    REG ADD %REG_ParentLogs%"\System" /v File /t REG_SZ /d "%SystemDrive%\Logs" /f
    REG ADD %REG_ParentLogs%"\System" /v AutoBackupLogFiles /t REG_SZ /d 1 /f
    echo "END - AUDIT LOGS"
    echo .
endlocal
EXIT /B 0